...
现在这个包不只是流程模板，还内置了一套可维护的聊天 UI 素材库。  
目录在：

```text
.ai-flow/references/UI素材库/
```

目前已收录：

- 苹果官方：`iPhone 16` 规格图、最新 `iOS` 风格参考
- 微信官方：消息列表、群聊、语音输入、语音转文字、表情搜索
- X 官方：图标、首页信息流、社区发现
- 小红书官方：图标、消息列表、笔记详情

设计原则：

- 素材优先使用公开官方来源或 App Store 公开截图
- 文件名统一使用中文，方便人工维护
- 每个素材在索引里都有来源链接与用途说明
- 缺少直接官方私信截图的场景，明确标记为“近似实现”，不伪装成官方还原

## 💬 聊天 UI 规则

除了素材，这个包还把聊天类界面的几条关键约束固化到了文档里：

- 私信里如果没有明确“自己”的身份信息，默认不生成回复
- 如果需要回复，第一人称头像允许自定义
- 聊天截图默认按 `iPhone 16 + 最新 iOS 风格` 对齐
- 顶部状态栏需要包含时间、电量、Wi‑Fi、蜂窝信号等系统信息
- 微信、X、小红书三类私信都要求优先贴近官方公开风格，不能拿泛 IM 样式冒充
